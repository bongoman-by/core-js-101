...
function getPolynom(...args) {
  ...
  return (x) => arr.reduce((prev, curr, index) => prev + curr * x ** index, 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const tmp = Array.from(args);
    const callStr = `${func.name}(${JSON.stringify(tmp).replace(
      /^.|.$/g,
      '',
    )})`;
    logFunc(`${callStr} starts`);
    const res = func.apply(this, tmp);
...
function partialUsingArguments(fn, ...args1) {
  return (...args2) => fn.apply(this, Array.prototype.concat(args1, args2));
}
